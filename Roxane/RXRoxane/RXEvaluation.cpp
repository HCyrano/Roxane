...
void RXEvaluation::load() {
    ...
    std::ifstream from("/Users/caussebruno/Documents/developpement/Roxane/build/eval_v10.1.bin", std::ios::binary);
    if(from) {
        ...
        for(unsigned int iStage = 0; iStage<60; iStage++) {
            ...
            //edge 8+6 id_patt = 6
            eval[iStage][6] = new short[4782969];
            from.read(reinterpret_cast<char*> (eval[iStage][6]), sizeof(short)*4782969);
            eval[iStage][6] += 4782969/2;

            //edge 2*(3+XC)) id_patt = 7
            ...
            // corner 4/4/3/2 id_patt = 12
            eval[iStage][12] = new short[1594323];
            from.read(reinterpret_cast<char*> (eval[iStage][12]), sizeof(short)*1594323);
            eval[iStage][12] += 1594323/2;
