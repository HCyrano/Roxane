...
void RXEvaluation::load() {
    ...
    if(from) {
        ...
        for(unsigned int iStage = 0; iStage<60; iStage++) {
            ...
            // corner 5/3/3/1/1 id_patt = 8
            eval[iStage][8] = new short[531441];
            from.read(reinterpret_cast<char*> (eval[iStage][8]), sizeof(short)*531441);
            ...
            //hyper diag id_patt = 10
            eval[iStage][10] = new short[59049];
            from.read(reinterpret_cast<char*> (eval[iStage][10]), sizeof(short)*59049);
            eval[iStage][10] += 59049/2;

            // corner 4/4/3/2 id_patt = 11
            eval[iStage][11] = new short[1594323];
            from.read(reinterpret_cast<char*> (eval[iStage][11]), sizeof(short)*1594323);
            eval[iStage][11] += 1594323/2;

            //edge 4/2/4 id_patt = 12
