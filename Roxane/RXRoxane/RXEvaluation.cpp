...
void RXEvaluation::load() {
    ...
    std::ifstream from("/Users/caussebruno/Documents/developpement/Roxane/build/weight_v7.bin", std::ios::binary);
    if(from) {
        ...
        for(unsigned int iStage = 0; iStage<60; iStage++) {
            ...
            eval[iStage][11] = new short[177147];
            from.read(reinterpret_cast<char*> (eval[iStage][11]), sizeof(short)*177147);
            eval[iStage][11] += 177147/2;
            
            // corner2*5 id_patt = 12
